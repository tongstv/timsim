...
class Paginator{
	...
	public function __construct($total=0,$mid_range=7,$ipp_array=array(10,25,50,100,"All")) {
		...
		$this->current_page = isset($url[1]) ? (int) $url[1] : 1 ; // must be numeric > 0
		...
		if($this->num_pages > 10) {
			$this->return = ($this->current_page > 1 And $this->total_items >= 10) ? "<li><a class=\"paginate\" href=\"$this->querystring?page=".($this->current_page-1)."\">Trước</a></li> ":"<li><span class=\"inactive\" href=\"#\">Trước</span></li> ";
			...
			for($i=1;$i<=$this->num_pages;$i++) {
				...
				if($i==1 Or $i==$this->num_pages Or in_array($i,$this->range)) $this->return .= ($i == $this->current_page And $this->items_per_page != "All") ? "<li class=\"active\"><a title=\"Go to page $i of $this->num_pages\" class=\"current\" href=\"javascript:void(0)\">[$i]</a> </li>\n":"<li><a class=\"paginate\" title=\"Go to page $i of $this->num_pages\" href=\"$this->querystring?page=$i\">$i</a></li>\n";
				if($this->range[$this->mid_range-1] < $this->num_pages-1 And $i == $this->range[$this->mid_range-1]) $this->return .= " ... ";
			}
			$this->return .= (($this->current_page < $this->num_pages And $this->total_items >= 10) And ($this->items_per_page != "All") And $this->current_page > 0) ? "<li><a class=\"paginate\" href=\"$this->querystring?page=".($this->current_page+1)."\">Tiếp theo</a></li>\n":"<li><span class=\"inactive\" href=\"#\">Tiếp theo</span></li>\n";

		} else	{
			for($i=1;$i<=$this->num_pages;$i++) {
				$this->return .= ($i == $this->current_page) ? "<li><a class=\"current\" href=\"#\">[$i]</a></li> ":"<li><a class=\"paginate\" href=\"$this->querystring?page=$i\">$i</a></li>";
			}
	...
	public function display_items_per_page() {
		...
		return "<span class=\"paginate\">Hiển thị</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&amp;ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>Row\n";
	}
	public function display_jump_menu() {
		...
		return "<li><span class=\"paginate\">Trang:</span></li><select class=\"paginate form-control input-sm\" style=\"width:50px\" onchange=\"window.location='$this->querystring?page='+this[this.selectedIndex].value+'';return false\">$option</select>\n";
	}
